...
describe('retryWithExponentialBackoff', () => {
  ...
  it('should retry promise', async () => {
    ...
    expect(mockLogger.error).toHaveBeenNthCalledWith(
      1,
      'Error in retryWithExponentialBackoff: first error'
    );
    expect(mockLogger.info).toHaveBeenNthCalledWith(
      1,
      expect.stringContaining('Retrying (attempt 1) in')
    );
    expect(mockLogger.error).toHaveBeenNthCalledWith(
      2,
      'Error in retryWithExponentialBackoff: second error'
    );
    expect(mockLogger.info).toHaveBeenNthCalledWith(
      2,
      expect.stringContaining('Retrying (attempt 2) in')
    );
    expect(mockFn).toHaveBeenCalled();
  });
});
