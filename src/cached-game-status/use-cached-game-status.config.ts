...
export const cachedGameStatusOptionsSchema = z.object({
  updateInterval: z.object({
    playersAndSquads: z.number().min(1).default(5).describe('--> about players and squads.'),
    layerInfo: z.number().min(1).default(60).describe('--> about layer info.'), // todo à confirmer, mais je pense que serveInfo done layers.
    serverInfo: z.number().min(1).default(30).describe('--> about server info.'),
  })
    .describe('Interval in seconds between each update of RCON provided data.\n' +
