...
const enabledSchema = pluginBaseOptionsSchema.extend({
  ...
  messages: z.object({
    ...
    noInGameAdminNotification: z.string().default('There are no in-game admins, however, an admin has been notified via Discord. Please wait for us to get back to you.'),
    adminInGameNotification: z.string().default('todo - default to english for now to handle plural...'),
    neutralAdminNotification: z.string().default('An admin has been notified. Please wait for us to get back to you.')
      .describe('Neutral message that will be shown if showInGameAdmins is false.'),
  }),
  showInGameAdmins: z.boolean().default(true).describe('Inform player if there are in-game admins.'),
