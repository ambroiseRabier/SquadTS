...
const schema = pluginBaseOptionsSchema
  .extend({
    messages: z
      .array(z.string().nonempty().includes('%attacker%').includes('%victim%'))
      .nonempty()
      .default(['%victim% just got knifed by %attacker%'])
      .describe(
    ...
    delay: z
      ...
      .min(0)
      .default(5)
      .describe('How many seconds to wait before broadcasting.'),
  })
  .describe('Broadcast to the whole server that someone just go knifed.');
