...
const DiscordServerStatus: SquadTSPlugin<DiscordServerStatusOptions> = async (server, connectors, logger, options) => {
  const { channelID } = options;
  const channel = await useDiscordChannel(connectors.discord, channelID);

  interval(options.updateFrequency * 1000).pipe(
    exhaustMap(async (data) => {
      const players = [
        `${server.info.a2sPlayerCount}`,
        (server.info.publicQueue + server.info.reserveQueue > 0) && `(+${server.info.publicQueue + server.info.reserveQueue})`,
