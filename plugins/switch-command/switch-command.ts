...
import {
  balanceIncreaseSwitch,
  SwitchRequest,
  unbalanceSwitch,
} from './src/switch-helpers';
...
const switchCommand: SquadTSPlugin<SwitchCommandConfig> = async (
  ...
  function trackPlayer(player: Player, date: Date) {
    ...
    if (player.teamID === '1') {
      const req = switchRequestToTeam2.find(
        (req) => req.eosID === player.eosID
      );
      if (req) {
      ...
      const req = switchRequestToTeam1.find(
        (req) => req.eosID === player.eosID
      );
      if (req) {
        req.date = date;
  ...
  server.chatEvents.command
    ...
    .subscribe(async (data) => {
      ...
      const ignoreRules = server.helpers.playerHasPermissions(
        data.player.eosID,
        [AdminPerms.Balance]
      );
      ...
      if (ignoreRules) {
        logger.info(
          `Switching admin ${data.player.nameWithClanTag} (${data.player.eosID})`
        );
      ...
      const team1Count = server.players.filter(
        (player) => player.teamID === '1'
      ).length;
      const team2Count = server.players.filter(
      ...
      if (ignoreCooldown || !hasCooldown(data.player.eosID)) {
        ...
        logger.debug(
          `Player ${data.player.nameWithClanTag} (${data.player.eosID}) is on cooldown`
        );
        const timeDiff =
          new Date().getTime() -
          playerOnCooldown.get(data.player.eosID)!.getTime();
  ...
  server.events.playerDisconnected.subscribe(async (data) => {
    switchRequestToTeam1 = switchRequestToTeam1.filter(
      (req) => req.eosID !== data.player.eosID
    );
  ...
  trackBalanceChange(server.players$)
    .pipe(
      filter(
        (newBalance) =>
          switchRequestToTeam1.length > 0 || switchRequestToTeam2.length > 0
      ),
  ...
  async function trySwitch(eosIDToAskForPatience?: string) {
    ...
    switchRequestToTeam1 = switchRequestToTeam1.filter(
      (req) =>
        now.getTime() - req.date.getTime() <= options.watchDuration * 1000
    );
    switchRequestToTeam2 = switchRequestToTeam2.filter(
      (req) =>
        now.getTime() - req.date.getTime() <= options.watchDuration * 1000
    ...
    switchRequestToTeam1 = sortRequests(
      switchRequestToTeam1,
      server.helpers.playerHasPermissions
    );
    switchRequestToTeam2 = sortRequests(
      switchRequestToTeam2,
      server.helpers.playerHasPermissions
    );
    ...
    const ids = balanceIncreaseSwitch(
      switchRequestToTeam1,
      switchRequestToTeam2
    );
    ...
    // Remove ids we switched
    switchRequestToTeam1 = switchRequestToTeam1.filter(
      (req) => !unbalancedIds.includes(req.eosID)
    );
    switchRequestToTeam2 = switchRequestToTeam2.filter(
      (req) => !unbalancedIds.includes(req.eosID)
    );
    ...
    for (let eosID of toBeSwitched) {
      ...
      const hasTeamChangePerm = server.helpers.playerHasPermissions(eosID, [
        AdminPerms.TeamChange,
      ]);
