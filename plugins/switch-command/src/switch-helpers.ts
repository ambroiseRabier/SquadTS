 ...
 */
export function balanceIncreaseSwitch(
  ...
  const balancePreservedAllowedSwitch = Math.min(
    reqTeam1.length,
    reqTeam2.length
  );
  ...
  for (let i = 0; i < balancePreservedAllowedSwitch; i++) {
    switchPlayers.push(reqTeam1[i].eosID, reqTeam2[i].eosID);
  }
...
export function unbalanceSwitch(p: {
  ...
  if (p.reqTeam1.length !== 0 && p.reqTeam2.length !== 0) {
    throw new Error(
      'At least reqTeam1 and reqTeam2 must be empty, first use balanceIncreaseSwitch.'
    );
  }

  const switchability = checkUnbalancedSwitchability(
    p.team1Count,
    p.team2Count,
    p.maxAllowedPlayerCountDiff
  );

  // We may be allowed up to 5 switch, but we may only have 2 players that want to switch
  const switchTeam1 = Math.min(
    switchability.maxTeam2MoveAllowed,
    p.reqTeam1.length
  );
  const switchTeam2 = Math.min(
    switchability.maxTeam1MoveAllowed,
    p.reqTeam2.length
  );
