...
const maxPlayerInSquad: SquadTSPlugin<MaxPlayerInSquadOptions> = (server: SquadServer, logger: Logger, options: MaxPlayerInSquadOptions) => {
    ...
    for (const [squadLead_eosID, transgressorDetails] of transgressors) {
      ...
      if (tooManyPlayers) {
        if (transgressorDetails.warnCount > options.maxWarnBeforeDisband) {
          await punish(sl, transgressorDetails, playersInSquad.length, playersInSquad.filter(p => p.eosID != sl.eosID));
          transgressors.delete(squadLead_eosID);
        } else {
  ...
  async function warn(squadLeader: PlayerSL, transgressorDetails: TransgressorDetails, playersInSquadLength: number) {
    const message = options.message
  ...
  async function punish(squadLeader: PlayerSL, transgressorDetails: TransgressorDetails, playersInSquadLength: number, otherPlayersInSquad: Player[]) {
    ...
    await Promise.all(otherPlayersInSquad.map((player) => server.rcon.warn(player.eosID, disbandMessage)))
