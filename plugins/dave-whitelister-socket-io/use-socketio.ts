 ...
 *       })
 */
export function useSocketIO(server: SquadServer, options: DaveWhitelisterSocketIoOptions, logger: Logger) {
  function setEmissions(socket: Socket) {
    ...
    subs.push(server.events.playerConnected.pipe(
      switchMap(
        (playerConnected) => {
          // Wait for nameWithClanTag to be set
          const waitForClanTag = server.players$.pipe(
            filter((players) => players.find(player => player.eosID === playerConnected.eosID)?.nameWithClanTag !== undefined),
            map(players => ({
              ...playerConnected,
    ...
    subs.push(server.events.playerDisconnected.pipe(
    ...
    subs.push(server.chatEvents.message.subscribe(data => {
  ...
  // const io = new Server(options.websocketPort); // Does not seem sufficient.
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('rcon.warn', (data: {steamID: string, message: string}) => {
      logger.debug(`rcon.warn ${util.inspect(data, false, null, true)}`);
      return server.rcon.warn(data.steamID, data.message);
    });
    socket.on('rcon.getListPlayers', (data) => {
      logger.debug(`rcon.getListPlayers ${util.inspect(data, false, null, true)}`);
